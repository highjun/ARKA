...
export const review = (base: string, head: string): Review => {
  ...
  const contracts = changed
    .map(({ path: file }) => ({ file, root: contractRoot(file) }))
    .filter((c): c is { file: string; root: string } => c.root !== undefined)
    .flatMap(({ file, root }) => changesIn(file.slice(root.length), at(base, file), at(head, file)));
