...
globalThis.__arka.meta = (() => {
  const M = {
    ...
    Shell: {
      ...
      props: {
        colorMode: { t: "enum", d: "light | dark. Primer `ThemeProvider` 로 그대로 간다" },
        isNarrow: { t: "boolean", d: "좁은 화면인가. 지금은 `data-narrow` 로만 실린다" },
        brand: { t: "slot", d: "제목 줄 왼쪽 — 마크와 작업 공간 이름" },
        center: { t: "slot", d: "제목 줄 가운데 — `CommandCenter`" },
        actions: { t: "slot", d: "제목 줄 오른쪽 — 빌드 표시 · 알림 종 · 밝기 토글" },
        sidebars: { t: "object", d: "레일 위 묶음 — `[{ id, title, iconId }]`. 없으면 사이드바가 통째로 안 뜬다" },
        activeSidebarId: { t: "string", d: "지금 열린 사이드바의 id. **활성은 위 묶음만 든다**" },
        sidebarTitle: { t: "string", d: '사이드바 제목 — "탐색기". 없으면 머리 줄이 안 선다' },
        sidebarContent: { t: "slot", d: "사이드바 본문. 비면 레일만 남기고 48px 로 접힌다" },
        sidebarActions: { t: "object", d: "사이드바 머리 오른쪽 단추들 — `[{ actionId, iconId, label }]`" },
        sidebarOpen: { t: "boolean", d: "사이드바가 열려 있다. 안 주면 스스로 든다" },
        sidebarResizable: { t: "boolean", d: "경계를 끌 수 있게 한다" },
        sidebarMinWidth: { t: "string", d: '끌어 줄일 수 있는 바닥 — 기본 "240px". 천장은 480px 고정' },
        sidebarWidthStorageKey: { t: "string", d: "폭을 기억할 `localStorage` 키" },
        bottoms: { t: "object", d: "아래 창들 — `[{ id, title, iconId, isActive }]`. 비면 아래 창이 없다" },
        bottomContent: { t: "slot", d: "열린 아래 창의 본문. 없으면 띠만 남는다" },
        children: { t: "slot", d: "가운데 편집 자리. `Tab` 이 온다" },
        overlays: { t: "slot", d: "위에 겹쳐 뜰 것 — 팔레트 · 토스트" },
        onSidebarSelect: { t: "action", d: "레일에서 고를 때" },
        onSettingsSelect: { t: "action", d: "레일 맨 아래 톱니를 누를 때" },
        onSidebarActionActivate: { t: "action", d: "사이드바 머리의 단추를 누를 때. 명령 id 를 준다" },
        onSidebarOpenChange: { t: "action", d: "사이드바를 여닫을 때" },
        onBottomSelect: { t: "action", d: "아래 창 탭을 누를 때" },
      },
    },
    TitleBar: {
    ...
    "Tab/Strip": {
      ...
      props: {
        tabs: { t: "object", d: "탭들 — `[{ id, kind, title, icon, Content, isDirty, isPreview }]`" },
        activeTabId: { t: "string", d: "지금 탭의 id. 없으면 `null`" },
        renderTabMenu: { t: "slot", d: "주면 탭마다 우클릭 메뉴가 생긴다" },
        overlay: { t: "slot", d: "띠 오른쪽 끝에 얹을 것 — `Tab/Actions` 가 설 자리" },
        onSelect: { t: "action", d: "탭을 누를 때" },
        onClose: { t: "action", d: "주면 닫기 × 가 생긴다" },
        onReorder: { t: "action", d: "주면 끌어 옮기기가 켜진다" },
        onPin: { t: "action", d: "주면 미리보기 탭을 더블클릭해 고정할 수 있다" },
      },
    },
    "Tab/Header": {
      ...
      props: {
        ...
        dirty: { t: "boolean", n: "tab.isDirty", d: "안 저장됨. 닫기 자리에 점이 뜬다" },
        preview: { t: "boolean", n: "tab.isPreview", d: "미리보기. 이름이 기울어진다" },
        title: { t: "string", n: "tab.title", d: '탭 이름 — "ThemeModel.ts". **`tab` 객체 하나로 받는다**' },
        iconId: { t: "enum", n: "tab.icon", d: "그릴 글리프. 확장이 `TabDescriptor` 에 담아 준다" },
        onSelect: { t: "action", d: "탭을 누를 때" },
        onClose: { t: "action", d: "주면 닫기 × 가 생긴다" },
      },
    ...
    "Tab/Group": {
      ...
      props: {
        tabs: { t: "object", d: "탭들 — `[{ id, kind, title, icon, Content, isDirty, isPreview }]`" },
        activeTabId: { t: "string", d: "지금 탭의 id. 없으면 `null`" },
        isNarrow: { t: "boolean", d: "좁은 화면이면 띠를 접는다" },
        content: { t: "slot", d: "고른 탭의 내용. 없으면 `TabRow.Content` 를 그린다" },
        emptyMessage: { t: "slot", d: '고른 탭이 없을 때 — 기본 "No selected tab"' },
    ...
    "Tab/Split": {
      ...
      props: {
        ...
        activePaneId: { t: "string", d: "지금 자리의 id. 그 그룹의 탭만 밝다" },
        renderContent: { t: "slot", d: "칸마다 그릴 것 — `(paneId, tabId)`" },
        onMove: { t: "action", d: "탭을 다른 그룹으로 떨어뜨릴 때" },
        onSplit: { t: "action", d: "탭을 가장자리에 떨어뜨려 가를 때" },
        onResize: { t: "action", d: "경계를 끌 때" },
        onSelect: { t: "action", d: "탭을 누를 때. `paneId` 가 첫 인자다" },
        onClose: { t: "action", d: "닫을 때" },
      },
    },
    "Tab/Actions": {
