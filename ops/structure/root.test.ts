...
const REASONS: Readonly<Record<string, string>> = {
  ...
  tools:
    "배포되지 않는 개발 도구. `ops`가 세 패키지를 **함께** 다스리는 것이라면 여기는 한 가지 일만 하는 것들이다 — Figma 시트를 다시 그리는 도구와, 바깥 제품의 UI를 컴포넌트 단위로 긁어 오는 캡처 도구. `ops`에 넣지 않은 이유는 그 패키지의 typecheck·lint가 남의 사이트와 플러그인 샌드박스에서 도는 코드까지 덮게 되어서다.",
