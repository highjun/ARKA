...
/**
 * **각 패키지의 `src/` 밖을 건드렸으면 PR 본문이 그것을 말해야 한다**(→ ADR 0003의 체크리스트).
...
/**
 * 파일 이름이든 그 폴더든, 본문이 가리키면 언급된 것으로 본다. **ADR과 태스크는 번호로도 받는다** —
 * 이 저장소가 그것을 인용하는 정본 형태가 `ADR 0010`·`TASK-47`이고, 그 번호가 실재하는지는
 * `ops/structure/`가 이미 본다.
 */
const mentioned = (file: string): boolean => {
  ...
  const adr = /^docs\/adr\/(?<number>\d{4})-/u.exec(file)?.groups?.["number"];
  if (adr !== undefined) candidates.push(`ADR ${adr}`);
  const task = /^docs\/tasks\/(?<number>\d{4})\.md$/u.exec(file)?.groups?.["number"];
  if (task !== undefined) candidates.push(`TASK-${String(Number(task))}`);
...
if (silent.length > 0) {
  console.error(
    `\nPR 본문이 위 ✗ ${silent.length}개를 말하지 않습니다 — 무엇을 왜 바꿨는지 본문에 적으세요(→ ADR 0003).`,
  );
  process.exit(1);
}
