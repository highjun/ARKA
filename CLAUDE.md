...
- 커밋 때 `commit-msg`가 밖의 변경을 세어 **커밋을 막는다.** 사용자에게 검토를 요청한다.
- 승인받은 뒤에만 지나간다: `git commit --trailer "Reviewed-by-human: <이름>" …`. **승인 없이 이 트레일러를 적지 않는다** — 우회가 `git log --grep`에 영구히 남는다.
