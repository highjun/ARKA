...
describe('ICommandCenterRegistry — 커맨드 실행', () => {
  ...
  it('when 조건이 거짓이면 매칭되지 않는다', () => {
    ...
    let executed = false;
    registry.registerCommand({ id: 'test.run', label: '실행', execute: () => (executed = true) });
    registry.registerKeybinding({ id: 'test.run.keybinding', keybinding: 'ctrl+j', actionId: 'test.run', when: () => false });
...
describe('ICommandCenterRegistry — registry 노출', () => {
  it('등록한 것을 registry로도 조회할 수 있다', () => {
    ...
    registry.registerContext({ id: 'test.flag', atom: fakeAtom(true) as Parameters<ICommandCenterRegistry['registerContext']>[0]['atom'] });
