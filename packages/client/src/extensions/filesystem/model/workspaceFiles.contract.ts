 ...
 */
export const testWorkspaceFilesContract = (name: string, setup: () => Promise<IWorkspaceFiles> | IWorkspaceFiles): void => {
  describe(`IWorkspaceFiles: ${name}`, () => {
    ...
    it('쓴 내용을 그대로 읽는다', async () => {
      ...
      expect(await files.read('a.txt')).toEqual({ path: 'a.txt', content: '안녕\n', truncated: false, encoding: 'utf8' });
    });
