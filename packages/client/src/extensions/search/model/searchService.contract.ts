...
export const testSearchServiceContract = (name: string, setup: () => SearchSetup | Promise<SearchSetup>): void => {
  describe(`ISearchService: ${name}`, () => {
    ...
    it("대소문자 무시로 경로순·줄순으로 찾고 줄·열은 1부터다", async () => {
      const { matches, truncated } = await s.service.search({
        query: "agent",
        path: "",
        regex: false,
        caseSensitive: false,
    ...
    it("정규식을 쓸 수 있고 리터럴은 이스케이프된다", async () => {
      expect(
        (await s.service.search({ query: "ag.nt ag", path: "", regex: true, caseSensitive: false })).matches,
      ).toHaveLength(1);
      expect(
        (await s.service.search({ query: "ag.nt ag", path: "", regex: false, caseSensitive: false })).matches,
      ).toHaveLength(0);
    });
    ...
    it("없으면 빈 결과다", async () => {
      const { matches, filesScanned } = await s.service.search({
        query: "zzz-none",
        path: "",
        regex: false,
        caseSensitive: false,
