...
describe("HttpSearchService", () => {
  ...
  it("조건을 쿼리로 싣고 계약대로 읽는다", async () => {
    ...
    expect(
      await createSearchServicePort().search({ query: "a b", path: "src", regex: true, caseSensitive: false }),
    ).toEqual(body);
  ...
  it("실패하면 상태를 담아 던진다", async () => {
    ...
    await expect(
      createSearchServicePort().search({ query: "a", path: "", regex: false, caseSensitive: false }),
