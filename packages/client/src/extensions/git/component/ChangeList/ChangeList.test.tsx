...
describe('ChangeList', () => {
  ...
  it('머리글 버튼은 전체에 적용한다', () => {
    const onAll = vi.fn();
    render(<ChangeList heading="변경 사항" entries={ENTRIES} action={{ ...action, onAll }} onSelect={() => undefined} />);
  ...
  implementsClassName((extra) => <ChangeList heading="변경" entries={ENTRIES} action={action} onSelect={() => undefined} {...extra} />);
  implementsDataComponent((extra) => <ChangeList heading="변경" entries={ENTRIES} action={action} onSelect={() => undefined} {...extra} />, 'ChangeList');
  implementsRef<HTMLUListElement>((extra) => <ChangeList heading="변경" entries={ENTRIES} action={action} onSelect={() => undefined} {...extra} />, HTMLUListElement);
  implementsNoA11yViolations(() => <ChangeList heading="변경" entries={ENTRIES} action={action} onSelect={() => undefined} />);
});
