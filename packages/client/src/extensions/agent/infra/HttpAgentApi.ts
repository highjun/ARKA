...
class HttpAgentApiAdapter implements IAgentApi {
  ...
  async createSession(title?: string): Promise<AgentSession> {
    return SessionResponse.parse(
      await this.#json("/api/agent/sessions", { method: "POST", body: title === undefined ? {} : { title } }),
    ).session;
  }
  ...
  async updateSession(
    ...
    return SessionResponse.parse(
      await this.#json(`/api/agent/sessions/${encodeURIComponent(id)}`, { method: "PATCH", body: patch }),
    ).session;
  }
  ...
  async startRun(
    ...
    input: string,
    mode: RunMode,
    { confirmWrites = true }: { readonly confirmWrites?: boolean } = {},
  ) {
    return RunResponse.parse(
      await this.#json(`/api/agent/sessions/${encodeURIComponent(sessionId)}/runs`, {
        method: "POST",
        body: { input, mode, confirmWrites },
      }),
    );
  }

  async provideInput(sessionId: SessionId, runId: string, requestId: string, text: string): Promise<void> {
    await this.#json(`/api/agent/sessions/${encodeURIComponent(sessionId)}/runs/${encodeURIComponent(runId)}/input`, {
      method: "POST",
      body: { requestId, text },
    });
  }
  ...
  async cancelRun(sessionId: SessionId, runId: string): Promise<void> {
    await this.#json(`/api/agent/sessions/${encodeURIComponent(sessionId)}/runs/${encodeURIComponent(runId)}`, {
      method: "DELETE",
    });
  }
