...
let seq = 0;
const ev = (partial: DistributiveOmit<AgentEvent, "seq" | "sessionId" | "at">): AgentEvent =>
  ({ ...partial, seq: ++seq, sessionId: "s", at: seq }) as AgentEvent;
...
describe("foldEvent", () => {
  ...
  it("생각 조각도 같은 blockId에 이어 붙는다", () => {
    const chat = fold(
      ev({ runId: "r", type: "thinking.delta", blockId: "b", text: "a" }),
      ev({ runId: "r", type: "thinking.delta", blockId: "b", text: "b" }),
      ev({ runId: "r", type: "thinking.done", blockId: "b" }),
    );
  ...
  it("툴 호출은 결과가 오면 채워진다", () => {
    const chat = fold(
      ev({ runId: "r", type: "tool.call", callId: "c", toolId: "echo", input: { x: 1 } }),
      ev({ runId: "r", type: "tool.result", callId: "c", output: { y: 2 }, isError: false }),
    );
  ...
  it("입력 요청은 대기 상태를 만들고, 답이 오면 사용자 메시지로 남고 대기가 풀린다", () => {
    const asked = fold(
      ev({ runId: "r", type: "run.started", mode: "action", input: "q?" }),
      ev({ runId: "r", type: "input.requested", requestId: "q", prompt: "이름?" }),
    );
    expect(asked).toMatchObject({
      runStatus: "waitingInput",
      pendingInput: { runId: "r", requestId: "q", prompt: "이름?" },
    });
  ...
  it("run.finished는 Run을 닫는다", () => {
    const chat = fold(
      ev({ runId: "r", type: "run.started", mode: "action", input: "x" }),
      ev({ runId: "r", type: "run.finished", status: "done" }),
    );
