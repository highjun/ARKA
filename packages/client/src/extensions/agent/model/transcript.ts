...
): readonly TranscriptItem[] => items.map((item) => (item.id === id ? update(item) : item));
...
export const foldEvent = (chat: SessionChat, event: AgentEvent): SessionChat => {
  ...
  switch (event.type) {
    ...
    case "assistant.delta":
      return {
        ...
        items: items.some((item) => item.id === event.messageId)
          ? replace(items, event.messageId, (item) =>
    ...
    case "assistant.done":
      return {
        ...base,
        items: replace(items, event.messageId, (item) => (item.kind === "assistant" ? { ...item, done: true } : item)),
      };
    case "thinking.delta":
      return {
        ...base,
        items: items.some((item) => item.id === event.blockId)
          ? replace(items, event.blockId, (item) =>
              item.kind === "thinking" ? { ...item, text: item.text + event.text } : item,
            )
    ...
    case "thinking.done":
      return {
        ...base,
        items: replace(items, event.blockId, (item) => (item.kind === "thinking" ? { ...item, done: true } : item)),
      };
    case "tool.call":
      return {
        ...base,
    ...
    case "tool.result":
      return {
        ...base,
        items: replace(items, event.callId, (item) =>
          item.kind === "tool" ? { ...item, output: event.output, isError: event.isError, done: true } : item,
        ),
      };
    case "input.requested":
      return {
        ...
        pendingInput: { runId: event.runId, requestId: event.requestId, prompt: event.prompt },
      };
    case "input.provided":
      return {
        ...base,
    ...
    case "run.error":
      return {
        ...base,
        items: [...items, { kind: "error", id: `error:${String(event.seq)}`, message: event.message, at: event.at }],
      };
