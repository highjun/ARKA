...
export type RunScript = (input: string, mode: RunMode, ids: { runId: string; next: () => string }) => readonly DistributiveOmit<EventInput, 'sessionId' | 'runId'>[];
...
export class MockAgentBackend implements IAgentApi, IAgentEvents {
  ...
  async updateSession(id: SessionId, patch: { readonly title?: string; readonly archived?: boolean }): Promise<AgentSession> {
    ...
    if (patch.archived !== undefined) this.#append({ sessionId: id, runId: null, type: 'session.archived', archived: patch.archived });
    const next = { ...current, ...(patch.title !== undefined ? { title: patch.title } : {}), ...(patch.archived !== undefined ? { archived: patch.archived } : {}), updatedAt: this.#now() };
