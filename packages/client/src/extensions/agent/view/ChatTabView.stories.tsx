...
const story = (state: Partial<ChatState>): Story => ({
  decorators: [
    (Story) => {
      ...
      container.register(
        ChatViewModelToken,
        singleton(() => viewModel(chat(state))),
      );
      return (
...
const ITEMS: readonly TranscriptItem[] = [
  ...
  },
  {
    kind: "assistant",
    id: "4",
    text: "tsconfig의 `paths`가 vite 설정과 어긋나 있습니다. 두 곳을 맞추겠습니다.",
    done: true,
    at: AT + 3000,
  },
];
...
export const Running: Story = story({
  items: [
    ...ITEMS.slice(0, 3),
    { kind: "assistant", id: "4", text: "tsconfig의 `paths`가", done: false, at: AT + 3000 },
  ],
  status: "running",
  canCancel: true,
...
export const Error: Story = story({
  items: [
    ...ITEMS.slice(0, 1),
    { kind: "error", id: "9", message: "ARKA_ANTHROPIC_API_KEY가 없습니다.", at: AT + 1000 },
  ],
  status: "error",
