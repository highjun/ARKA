 ...
 */
export const ChatSessionsView = ({ onOpenTab }: { readonly onOpenTab: (tab: { readonly id: string; readonly kind: string; readonly title: string }) => void }) => {
  ...
  return (
    <SessionList
      ...
      heading="에이전트"
      sessions={viewModel.sessions.map((session) => ({ id: session.id, title: session.title, status: session.status ?? undefined, timestamp: session.timestamp, archived: session.archived }))}
