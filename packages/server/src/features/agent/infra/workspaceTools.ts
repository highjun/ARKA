...
const DEFINITIONS: readonly AgentToolDefinition[] = [
  {
    name: "list_directory",
    description:
      "워크스페이스의 디렉터리 하나를 나열한다. 경로는 워크스페이스 루트 기준 상대 경로이고 루트는 빈 문자열이다.",
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: { path: { type: "string" }, type: { type: "string", enum: ["file", "dir"] } },
      required: ["path", "type"],
      additionalProperties: false,
    },
  },
      ...
      switch (name) {
        ...
        case "create_entry": {
          ...
          if (type !== "file" && type !== "dir")
            return { output: { error: "type must be file or dir" }, isError: true };
...
export const isApproval = (answer: string): boolean =>
  /^(?:예|네|응|허용|승인|y|yes|ok|okay)\s*[.!]?$/iu.test(answer.trim());
