...
describe('loadConfig', () => {
  ...
  describe("loadConfig", () => {
    it("기본값은 포트 3000, 루프백, 정적 서빙 없음이다", async () => {
      const config = await loadConfig({ ARKA_WORKSPACE: path.join(base, "ws"), ARKA_DATA_DIR: path.join(base, "data") });
    ...
    it("기능 조각의 오류도 같은 ConfigError로 나온다 — 관문이 하나다", async () => {
      await expect(loadConfig(env({ ARKA_WORKSPACE: path.join(base, "ws"), ARKA_AGENT_RUNNER: "anthropic" }))).rejects.toThrow(ConfigError);
      await expect(loadConfig(env({ ARKA_WORKSPACE: path.join(base, "ws"), ARKA_AGENT_RUNNER: "anthropic" }))).rejects.toThrow(/ARKA_ANTHROPIC_API_KEY/u);
    });

    it("기능 조각이 합쳐진다 — 실행기를 켜면 설정에 실려 나온다", async () => {
      const config = await loadConfig(env({ ARKA_WORKSPACE: path.join(base, "ws"), ARKA_AGENT_RUNNER: "anthropic", ARKA_ANTHROPIC_API_KEY: "sk-test" }));
      expect(config.agent).toEqual({ runner: "anthropic", apiKey: "sk-test", model: "claude-opus-5" });
    });

    it("데이터 디렉터리가 없으면 만든다", async () => {
      const config = await loadConfig({ ARKA_WORKSPACE: path.join(base, "ws"), ARKA_DATA_DIR: path.join(base, "fresh/nested") });
      expect(config.dataDir).toBe(path.join(base, "fresh/nested"));
    });

    it("데이터 디렉터리를 만들 수 없으면 ConfigError를 던진다", async () => {
      await expect(loadConfig({ ARKA_WORKSPACE: path.join(base, "ws"), ARKA_DATA_DIR: path.join(base, "file.txt", "x") })).rejects.toThrow(/ARKA_DATA_DIR/u);
    });
    ...
    it("워크스페이스가 파일이면 ConfigError를 던진다", async () => {
      await expect(loadConfig(env({ ARKA_WORKSPACE: path.join(base, "file.txt") }))).rejects.toThrow(/not a directory/u);
    });

    it("포트가 숫자가 아니면 ConfigError를 던진다", async () => {
      await expect(loadConfig(env({ ARKA_WORKSPACE: path.join(base, "ws"), ARKA_PORT: "abc" }))).rejects.toThrow(/ARKA_PORT/u);
    });

    it("포트 범위를 벗어나면 ConfigError를 던진다", async () => {
      await expect(loadConfig(env({ ARKA_WORKSPACE: path.join(base, "ws"), ARKA_PORT: "70000" }))).rejects.toThrow(ConfigError);
    });
  ...
  describe("빈 문자열은 없음이다", () => {
    it("빈 값을 준 옵셔널 변수가 부팅을 막지 않는다 — docker의 `ENV X=$ARG`가 그렇게 준다", async () => {
      const config = await loadConfig({ ARKA_WORKSPACE: path.join(base, "ws"), ARKA_GIT_SHA: "", ARKA_ANTHROPIC_API_KEY: "" });
    ...
    it("기본값이 있는 것도 빈 값이면 기본으로 떨어진다", async () => {
      const config = await loadConfig({ ARKA_WORKSPACE: path.join(base, "ws"), ARKA_PORT: "", ARKA_HOST: "", ARKA_AGENT_RUNNER: "" });
