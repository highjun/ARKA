 ...
 */
export const createRequestLog = (log: Logger, { skip = ["/api/health"] }: { skip?: readonly string[] } = {}): MiddlewareHandler => {
  return async (c, next) => {
    const started = performance.now();
    await next();
    if (skip.includes(c.req.path)) return;
    const fields = { method: c.req.method, path: c.req.path, status: c.res.status, ms: Math.round(performance.now() - started) };
