...
- **선언 위에 주석을 달면 `/** */`로 쓴다** — `//`·`/* */`가 붙어 있으면 전환한다. 비공개 선언에 문서 블록을 **요구하지는 않는다**.
...
- 주석을 계약(`type`·`interface`·값 `const`)에만 허용하기 — 강제는 되지만 **공개 면에 문서가
  없는 것을 방치한다.**
- **비공개 선언에도 문서 블록 요구**(2026-09-14까지의 결정) — TSDoc·JSDoc은 **공개 면**을 문서화하는 도구이고 비공개까지 요구하는 표준은 없다. 요구하면 이름을 되풀이하는 블록이 늘어 같은 ADR의 `informative-docs`와 맞선다.
- `flat/recommended-tsdoc` 프리셋 — `require-param` 등 30개 남짓을 함께 켠다. 규칙에는 그것을
  낳은 결정이 있어야 하므로 손으로 고른다.

## 대가:
- **공개 면을 늘릴 때마다 문서를 쓴다.**
- 플러그인이 는다. `ops`가 그것을 다 물고 있다.
- **`//` 4줄 상한은 바깥에 표준이 없다** — 그래서 이 저장소의 유일한 자작 규칙(`arka/max-comment-lines`)을 낳았다. 그래도 남기는 이유는 그 상한이 배경 설명을 실제로 ADR로 밀어냈고, 대안인 리뷰가 더 약해서다(→ [ADR 0011](0011-lint-off-the-shelf.md)).
