...
- 패키지 간 참조는 workspace 패키지명으로 한다 — `import { URI } from "contracts"`. pnpm이 만든 심볼릭 링크와 `contracts`의 `main` 필드로 해석되므로 tsc·vitest·node·번들러가 모두 같은 방식으로 찾는다.
...
| 무엇이 지키나 | 어디 |
|---|---|
| 린트 | zone — `contracts`가 `client`·`server`를 import 금지 |
| 린트 | zone — `client`↔`server` 상호 import 금지 |
| 린트 | `lint:config` — tsconfig `paths` 금지. 기각 항목을 직접 막는다 |
| 아무도 | **"패키지명으로 가져온다"** — 상대경로 `../../../contracts/src/…`는 zone을 통과한다 |
