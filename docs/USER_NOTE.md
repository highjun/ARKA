...
- [ ] **LLM API 키** — 실제 실행기(`AnthropicRunner`)는 구현됐고 키가 있을 때만 켜진다. 서버 환경변수 `ADE_ANTHROPIC_API_KEY`(compose는 `.env` 파일)로 넘긴다. 모델은 `ADE_ANTHROPIC_MODEL`(기본 `claude-opus-5`). 키를 리포에 넣지 않는다. 키 없이 실제 API에 대고 검증한 적이 없으므로 처음 켜면 `docker logs`에서 `agent.runner` 줄과 첫 Run의 `run.error`를 확인한다.
...
- 커밋 메시지는 **한글 자연문** — 첫 줄은 무엇을 왜 했는지, 본문에 "결정한 것 / 확인 필요". Conventional Commits(`feat:` 접두)는 쓰지 않는다 — 주석·문서를 한글로 쓰는 결정(ADR 0002)과 일관되게, 그리고 접두어가 리뷰에 주는 정보가 없어서다. 실제 이력이 이미 그 형태다.
