...
`import-x/no-relative-packages`가 client·server에 있었고 contracts에만 빠져 있었다. 그 한 곳을
채워 세 패키지가 같은 선이 됐다. **fixture 는 두지 않는다** — 기성 규칙이라 그 테스트는 배포자가
든다(→ [ADR 0011](../adr/0011-lint-off-the-shelf.md)). 일부러 깨뜨려 무는 것을 확인했다.
