...
`§4`·`§5`·`§6`·`§8`·`§16`은 이 저장소 밖에 있는 원본 아키텍처 결정 문서(사용자가 저장소 밖에 보관)의 절 번호다. 그 문서는 리포에 넣지 않는다 — 한 번 통째로 넣었다가 리뷰 불가로 롤백한 전례가 있고, 결정의 요지는 각 ADR이 스스로 담고 있다. **지금의 ADR은 §n을 인용하지 않는다**(인용하던 것은 2026-09-12 아카이브와 함께 지워졌다). 갈린 자리는 다음과 같다.
...
| §8 | 디자인 시스템 | [ADR 0009](adr/0009-primer-first.md) — Primer 우선, 헤드리스만 Radix |
