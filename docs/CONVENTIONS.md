...
- 루트에 놓이는 것은 **패키지들이 공유하는 것**이어야 한다. 아니면 그것을 다스리는 패키지 안으로 들여보낸다 — 설정도 테스트도. → [ADR 0002](adr/0002-repo-root.md)
- 루트도 의미 단위로 묶는다 — `packages/`는 구현(배포된다), `ops/`는 운영 스크립트(배포되지 않는다), `docs/`는 글, `.output/`은 산출물. 항목별 뜻은 ADR의 트리에 있다.
