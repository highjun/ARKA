...
에이전트가 작업할 때 지켜야 할 규칙. 왜 그렇게 정했는지는 그 규칙을 들인 PR 본문에 있다.
...
이 제품이 풀려는 문제를 개발 과정에서 되풀이하지 않는다.

- **못 알아들었으면 진행하지 말고 확인한다.** 추측으로 진행하는 것이 가장 크게 잘못된다.
- **요청한 것만 한다.** 작업 중 발견한 산발적 문제를 함께 고치지 않는다 — `docs/tasks/`에 짧게 남기고 넘어간다.
- **현재 위치를 알려준다.** 마일스톤 기준으로 지금 어디고 무엇이 끝났고 무엇이 남았는지 함께 전한다.
- **설명은 짧게, 용어는 기존 것으로.** 새 용어를 만들어 쓰지 않는다 — 관례어를 먼저 찾는다.
- **사용자의 지식 수준에 맞춰 설명한다.** 동의보다 반박이 필요한 자리에서는 반박한다.

## 검토

사용자가 코드를 읽고 의도대로 되었는지 판정하는 절차다. 판정은 사용자만 한다 — 에이전트는 자료를 준비할 뿐 Accept/Reject를 스스로 정하지 않는다.

거기서 나온 것은 **둘 중 하나로만** 나간다 — 결정이면 PR 본문, 할 일이면 `docs/tasks/`. **중간 문서를 두지 않는다.** 진행 상황·후보·계획을 담는 산문 문서를 세 번 만들었다가 세 번 다 낡혔다(`REVIEW_CHECKLIST`·`ADR_CANDIDATE`·`lint-plan`). 낡는 이유는 같다 — 아무도 검사하지 않는 목록이라서다.
...
- **결정과 그 이유는 PR 본문에 쓴다.** squash 머지라 본문이 `main`의 기록이 된다 — 결정을 담는 별도 문서를 두지 않는다.
- **규칙이 된 것은 그 규칙을 강제하는 설정·테스트 옆에 한 줄로 적는다.** 문장과 검사기가 같은 파일에서 함께 산다 — 떨어뜨리면 문장만 남아 낡는다.
- **강제할 것이 없는 규칙은 규칙이 아니다.** 검사기를 만들거나 규칙을 버린다. 사람만 판정할 수 있는 것은 그렇다고 적는다.
...
- **번호 파일 하나에 할 일 하나**(`docs/tasks/0040.md`). 새로 남기는 것은 `pnpm run task create "<제목>"`이다.
- **`status`는 셋뿐이다** — 시작하면 `In Progress`, 끝나면 `Done`. `ops/structure/docs.test.ts`가 그 어휘를 강제한다.
- **목록은 `pnpm run task list`가 생성한다.** 유지하는 목록은 낡는다 — 산문 백로그를 다른 문서에 쌓지 않는다.

## 주석·문서

- 한글로 쓴다. 코드 식별자와 런타임 문자열(`throw new Error(...)` 등)만 영문.
- **"현재 코드는 어떻다"를 문서에 캐시하지 않는다.** 코드가 그 격차를 메운 순간 **역방향 거짓말**이 된다 — 이 저장소가 세 번 겪었다. 격차는 할 일 태스크(`docs/tasks/`)로 나간다.
- **마크다운의 문서 인용은 상대 경로 링크다** — 맨경로를 쓰지 않는다. 파일을 옮기면 조용히 깨지고, `ops/structure/reference.test.ts`가 링크만 본다.
- **주석은 자리가 형태와 필수 여부를 정한다.**
  ...
  - 배경 설명은 주석에 쌓지 않는다. 왜 이 모양인지는 그것을 들인 PR 본문에 있다.
...
- **사용자만 할 수 있는 일은 `docs/USER_NOTE.md` 맨 위 "사용자가 해야 할 일"에 적는다** — sudo, 클라우드 콘솔, 비밀값. **이 파일은 추적하지 않는다**(사용자 개인 메모다). 규칙이 될 내용은 여기 규약으로 옮긴다.
...
- 루트도 의미 단위로 묶는다 — `packages/`는 구현(배포된다), `ops/`는 운영 스크립트(앱에 딸려 나가지 않는다 — 다만 `ops/deploy/`가 배포를 수행한다), `docs/`는 글, `.output/`은 산출물. 항목별로 왜 전역인지는 `ops/structure/root.test.ts`의 표가 든다.
